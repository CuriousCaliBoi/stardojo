...
namespace ActionSpace.actions
{
    ...
    public static class Actions
    {
        ...
        {
            // Initialize with back buffer size, not viewport size
            int backBufferWidth = Game1.graphics.GraphicsDevice.PresentationParameters.BackBufferWidth;
            int backBufferHeight = Game1.graphics.GraphicsDevice.PresentationParameters.BackBufferHeight;
            pixelData = new byte[backBufferWidth * backBufferHeight * 4];
        }
        ...
        public static void updatePixelData(Mod mod)
        {
            try
            {
                ...
                // Use back buffer dimensions instead of viewport (back buffer = actual window size)
                int backBufferWidth = Game1.graphics.GraphicsDevice.PresentationParameters.BackBufferWidth;
                int backBufferHeight = Game1.graphics.GraphicsDevice.PresentationParameters.BackBufferHeight;
                int expectedSize = backBufferWidth * backBufferHeight * 4;
                
                // Resize buffer if needed
                if (pixelData == null || pixelData.Length != expectedSize)
                {
                    pixelData = new byte[expectedSize];
                    currentViewport[0] = backBufferWidth;
                    currentViewport[1] = backBufferHeight;
                }
